...
class HanabiEnv(Environment):
  ...
  def reset(self):
    ...
    obs = self._make_observation_all_players() # Make an observation for all players, which includes the current player and the player observations for all players. This is the full state of the game, including all hands, and should not be passed to the agents.
    obs["current_player"] = self.state.cur_player() # Add current player to observation
    return obs
    ...
    if log_file:
      with open('logs/'+log_file, 'a') as f, open('logs/gameplay/'+log_file, 'a') as f_gameplay:
        ...
        if move_type== pyhanabi.HanabiMoveType.PLAY:
          card_colour = pyhanabi.color_idx_to_char(last_move.color())
          card_rank = last_move.rank()
          f.write('Card played: Rank {}, Color {}\n'.format(card_rank, card_colour))
          f_gameplay.write('Card played: Rank {}, Color {}\n'.format(card_rank, card_colour))

        elif move_type == pyhanabi.HanabiMoveType.DISCARD:
          card_colour = pyhanabi.color_idx_to_char(last_move.color())
          card_rank = last_move.rank()
          f.write('Card discarded: Rank {}, Color {}\n'.format(card_rank, card_colour))
          f_gameplay.write('Card discarded: Rank {}, Color {}\n'.format(card_rank, card_colour))
